...
const biography = 'Há 7 anos, meu trabalho parte de uma escuta atenta e de escolhas que preservam a identidade. Mais do que transformar, acredito em revelar uma beleza natural, elegante e coerente com quem você é.';
...
function BrandSeal({ id }: { id: string }) {
  ...
  return <svg className={styles.seal} viewBox="0 0 120 120" aria-hidden="true" focusable="false">
    ...
    <text className={styles.sealText} textLength="280" lengthAdjust="spacing">
      <textPath href={`#${pathId}`} startOffset="1%">ALTA ESTÉTICA · JÉSSICA MARTINS · </textPath>
    </text>
    <text className={styles.sealMark} x="60" y="70" textAnchor="middle">jm.</text>
  ...
  return <section id="sobre" ref={root} className={styles.root} aria-labelledby="author-title">
    ...
    <div className={styles.stage}>
      ...
      <article className={styles.card} data-author-motion="card">
        <p className={styles.cardLabel}>DRA. JÉSSICA MARTINS · CRBM 02818</p>
      ...
      <svg className={`${styles.thread} ${styles.threadDesktop}`} viewBox="0 0 1600 110" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        <defs>
          <radialGradient id={`${uid}-author-pearl`} cx="30%" cy="25%">
            <stop stopColor="#f7eee7" />
            <stop offset=".5" stopColor="#c08462" />
            <stop offset="1" stopColor="#7f3824" />
          </radialGradient>
        </defs>
    ...
    <footer className={styles.closing}>
      ...
      <div className={styles.closingRow}>
        <div className={styles.closingMonogram} aria-hidden="true"><strong>jm.</strong><span /></div>
